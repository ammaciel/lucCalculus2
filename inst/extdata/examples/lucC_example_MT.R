...
lucC_create_RasterBrick(path_open_GeoTIFFs = "~/Desktop/Classi_MT_SVM/raster_mt_by_year_2004", path_save_RasterBrick = "~/Desktop")

# ------------- define variables to use in sits -------------
# open files
file <- c("~/Desktop/raster_mt_by_year_2004.tif")
file
...
raster.tb <- sits::sits_coverage(files = file, name = "MT", timeline = timeline, bands = "ndvi")
raster.tb
...
lucC_save_GeoTIFF(raster_obj = rb_sits,
                  ...
                  path_raster_folder = "~/Desktop/Classi_MT_SVM/raster_mt_by_year_2004Sec")
...
# create a RasterBrick from individual raster saved previously
lucC_create_RasterBrick(path_open_GeoTIFFs = "~/Desktop/Classi_MT_SVM/raster_mt_by_year_2004Sec", path_save_RasterBrick = "~/Desktop")

# ------------- define variables to use in sits -------------
# open files with new pixel secondary vegetation
file <- c("~/Desktop/raster_mt_by_year_2004Sec.tif")
file
...
#library(sits)
# create a RasterBrick metadata file based on the information about the files
raster.tb <- sits::sits_coverage(files = file, name = "MTSec", timeline = timeline, bands = "ndvi")
raster.tb
...
system.time(
  for(i in seq_along(classes)){
    print(classes[i])
    temp <- lucC_pred_convert(raster_obj = rb_sits, raster_class1 = classes[i],
                              time_interval1 = c("2001-09-01","2001-09-01"), relation_interval1 = "equals",
                              raster_class2 = "Forest",
