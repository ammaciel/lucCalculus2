...
#----------------------------
lucC_plot_bar_events3 <- function(data_mtx = NULL, data_frequency = NULL, custom_palette = FALSE, RGB_color = NULL, pixel_resolution = 250, relabel = FALSE, original_labels = NULL, new_labels = NULL, legend_text = "Legend:", column_legend = 2, side_by_side = FALSE){
  ...
  g <- ggplot2::ggplot(mapBar,aes(x=mapBar$Var1, y=mapBar$Freq, fill=mapBar$Var2)) +
    ...
    annotate("text", x = "2009", y = 1830000, label = "Soy Moratorium") +
    geom_segment(aes(x = "2008", xend = "2016", y = 1800000, yend = 1800000),
                 #arrow=arrow(length=unit(0.30,"cm"), angle = 20, ends = "both", type = "closed"), # length=unit(0.30,"cm") linetype="dashed",
                 linetype="dashed", color = "black") +
