...
for (y in 1:length(all.the.files)) {
  ...
  class1 <- c("Pasture")
  classes <- c("Soy", "Secondary_Vegetation", "Cerrado", "Degradation") #
  ...
  lucC_save_raster_result(raster_obj = rb_sits, data_mtx = direct_transi.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/DLUCPastSoySV/", file_name, sep = ""), as_RasterBrick = FALSE)

  message("Prepare image 2 ...\n")
  lucC_save_raster_result(raster_obj = rb_sits, data_mtx = direct_transi.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/DLUCPastSoySV/", file_name, sep = ""), as_RasterBrick = TRUE)
...
#save to rda file
save(number_P_Soy_SV, file = "~/TESTE/MT/DLUCPastSoySV/number_P_Soy_SV.rda")
...
load(file = "~/TESTE/MT/DLUCPastSoySV/number_P_Soy_SV.rda")
...
write.table(x = measuresPast_SoySV, file = "~/TESTE/MT/DLUCPastSoySV/measuresPast_SoySV.csv", quote = FALSE, sep = ";", row.names = FALSE)

save(measuresPast_SoySV, file = "~/TESTE/MT/DLUCPastSoySV/measuresPast_SoySV.rda")
