...
for (y in 1:length(all.the.files)) {
  ...
  lucC_save_raster_result(raster_obj = rb_sits, data_mtx = direct_transi.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/DLUCCerradoOthers/", file_name, sep = ""), as_RasterBrick = TRUE)

  message("Prepare image 2 ...\n")
  lucC_save_raster_result(raster_obj = rb_sits, data_mtx = direct_transi.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/DLUCCerradoOthers/", file_name, sep = ""), as_RasterBrick = FALSE)
...
lucC_merge_rasters(path_open_GeoTIFFs = "~/TESTE/MT/DLUCCerradoOthers/All_blocks_Cerrado_others", number_raster = 4, pattern_name = "New_New_New_Raster_Splitted_", is.rasterBrick = TRUE)
# save each layer of brick as images
lucC_save_rasterBrick_layers(path_name_GeoTIFF_Brick = "~/TESTE/MT/DLUCCerradoOthers/All_blocks_Cerrado_others/Mosaic_New_New_New_Raster_Splitted_.tif")
