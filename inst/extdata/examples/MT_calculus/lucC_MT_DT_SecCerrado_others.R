...
number_SecCerrado_others <- list(NULL)
...
for (y in 1:length(all.the.files)) {
  ...
  class1 <- c("Secondary_Cerrado")
  classes <- c("Pasture", "Soy", "Secondary_Vegetation", "Cerrado", "Degradation") #
  ...
  number_SecCerrado_others[[y]] <- direct_transi.df

  message("Prepare image 1 ...\n")
  lucC_save_raster_result(raster_obj = rb_sits, data_mtx = direct_transi.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/DLUCSecCerradoOthers/", file_name, sep = ""), as_RasterBrick = TRUE)

  message("Prepare image 2 ...\n")
  lucC_save_raster_result(raster_obj = rb_sits, data_mtx = direct_transi.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/DLUCSecCerradoOthers/", file_name, sep = ""), as_RasterBrick = FALSE)

  # clear environment, except these elements
  rm(list=ls()[!(ls() %in% c('all.the.files', "start.time", "number_SecCerrado_others"))])
  gc()
...
#save to rda file
save(number_SecCerrado_others, file = "~/TESTE/MT/DLUCSecCerradoOthers/number_SecCerrado_others.rda")
...
load(file = "~/TESTE/MT/DLUCSecCerradoOthers/number_SecCerrado_others.rda")

output_freq <- lucC_extract_frequency(data_mtx.list = number_SecCerrado_others, cores_in_parallel = 6)
output_freq
...
# Compute values
measuresSecCerrado_others <- lucC_result_measures(data_frequency = output_freq, pixel_resolution = 231.656)
measuresSecCerrado_others

write.table(x = measuresSecCerrado_others, file = "~/TESTE/MT/DLUCSecCerradoOthers/measuresSecCerrado_others.csv", quote = FALSE, sep = ";", row.names = FALSE)

save(measuresSecCerrado_others, file = "~/TESTE/MT/DLUCSecCerradoOthers/measuresSecCerrado_others.rda")
...
lucC_merge_rasters(path_open_GeoTIFFs = "~/TESTE/MT/DLUCSecCerradoOthers/All_blocks_SecCerrado_others", number_raster = 4, pattern_name = "New_New_New_Raster_Splitted_", is.rasterBrick = TRUE)
# save each layer of brick as images
lucC_save_rasterBrick_layers(path_name_GeoTIFF_Brick = "~/TESTE/MT/DLUCSecCerradoOthers/All_blocks_SecCerrado_others/Mosaic_New_New_New_Raster_Splitted_.tif")
