...
number_Soy_before_2008 <- list(NULL)
...
for (y in 1:length(all.the.files)) {
  ...
  message("Start Compute Soybean Before 2008 ...\n")
  ...
  for(x in 2:length(timeline2)){
    ...
    for( z in 1:length(classesDegPas)){
      ...
      pasture.df <- lucC_pred_holds(raster_obj = raster.data, raster_class = classesDegPas[z], #Degradation, Pasture
...
#save to rda file
save(number_Soy_before_2008, file = "~/TESTE/MT/Calc_Soybean_Before_2008/number_Soy_before_2008.rda")
...
load(file = "~/TESTE/MT/Calc_Soybean_Before_2008/number_Soy_before_2008.rda")

output_freq <- lucC_extract_frequency(data_mtx.list = number_Soy_before_2008, cores_in_parallel = 6)
output_freq
...
# Compute values
measuresSoy_Before_2008 <- lucC_result_measures(data_frequency = output_freq, pixel_resolution = 231.656)
measuresSoy_Before_2008

write.table(x = measuresSoy_Before_2008, file = "~/TESTE/MT/Calc_Soybean_Before_2008/measuresSoy_Before_2008.csv", quote = FALSE, sep = ";", row.names = FALSE)

save(measuresSoy_Before_2008, file = "~/TESTE/MT/Calc_Soybean_Before_2008/measuresSoy_Before_2008.rda")
