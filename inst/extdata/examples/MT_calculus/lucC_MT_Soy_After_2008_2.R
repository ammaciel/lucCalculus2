...
measuresSoy_After_2008 <- list(NULL)
...
for (y in 1:length(all.the.files)) {
  ...
  for(x in 2:length(timeline2)){
    ...
    for( z in 1:length(classesDegPas)){
      ...
      soybean.df <- lucC_pred_holds(raster_obj = raster.data, raster_class = "Soy", #"Pasture",
      ...
      temp <- lucC_relation_precedes(soybean.df, fores_past.temp) #, forest.df)
  ...
  measuresSoy_After_2008[[y]] <- soybean_after.df
  ...
  rm(list=ls()[!(ls() %in% c('all.the.files', "start.time", "measuresSoy_After_2008"))])
  gc()
...
load(file = "~/TESTE/MT/Calc_Soybean_After_2008/measuresSoy_After_2008.rda")

output_freq <- lucC_extract_frequency(data_mtx.list = measuresSoy_After_2008, cores_in_parallel = 6)
output_freq
