...
number_Past_after_2008 <- list(NULL)
...
for (y in 1:length(all.the.files)) {
  ...
  message("Start Compute Cerrado and Pasture After 2008 ...\n")
  ...
  number_Past_after_2008[[y]] <- soybean_after.df

  message("Prepare image 1 ...\n")
  lucC_save_raster_result(raster_obj = raster.data, data_mtx = soybean_after.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/Calc_Past_After_2008/", file_name, sep = ""), as_RasterBrick = FALSE)

  message("Prepare image 2 ...\n")
  lucC_save_raster_result(raster_obj = raster.data, data_mtx = soybean_after.df, timeline = timeline, label = label2, path_raster_folder = paste0("~/TESTE/MT/Calc_Past_After_2008/", file_name, sep = ""), as_RasterBrick = TRUE)

  # clear environment, except these elements
  rm(list=ls()[!(ls() %in% c('all.the.files', "start.time", "number_Past_after_2008"))])
  gc()
...
#save to rda file
save(number_Past_after_2008, file = "~/TESTE/MT/Calc_Past_After_2008/number_Past_after_2008.rda")
...
load(file = "~/TESTE/MT/Calc_Past_After_2008/number_Past_after_2008.rda")

output_freq <- lucC_extract_frequency(data_mtx.list = number_Past_after_2008, cores_in_parallel = 6)
output_freq
...
# Compute values
measuresPast_After_2008 <- lucC_result_measures(data_frequency = output_freq, pixel_resolution = 231.656)
measuresPast_After_2008

write.table(x = measuresPast_After_2008, file = "~/TESTE/MT/Calc_Past_After_2008/measuresPast_After_2008.csv", quote = FALSE, sep = ";", row.names = FALSE)

save(measuresPast_After_2008, file = "~/TESTE/MT/Calc_Past_After_2008/measuresPast_After_2008.rda")
...
lucC_merge_rasters(path_open_GeoTIFFs = "~/TESTE/MT/Calc_Past_After_2008/All_blocks_Past_After", number_raster = 4, pattern_name = "New_New_Raster_Splitted_", is.rasterBrick = TRUE)
# save each layer of brick as images
lucC_save_rasterBrick_layers(path_name_GeoTIFF_Brick = "~/TESTE/MT/Calc_Past_After_2008/All_blocks_Past_After/Mosaic_New_New_Raster_Splitted_.tif")
